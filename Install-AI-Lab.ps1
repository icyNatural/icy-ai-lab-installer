...
# Resolve Full Absolute Script Path & Working Directory
...
if (-not (Test-IsAdmin)) {
    ...
    # Build escaped argument string preserving paths with spaces or parentheses, keeping window open with -NoExit
    $argList = "-NoExit -NoProfile -ExecutionPolicy Bypass -File `"$resolvedScriptPath`" -InstallRoot `"$InstallRoot`""
